...
function envsource
    ...
    #First we save the number of arguments
    ...
    if test $num_of_args -eq 2 -a (string match -q -- - $argv[1][1] -o $num_of_args -eq 1 -a ! string match -q -- - "$argv[2]")
    ...
    else if test $num_of_args -eq 1 -a (! string match -q -- - $argv[1])
        set options -gx
        set file $argv[1]
